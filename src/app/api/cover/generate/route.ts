...
## 原则
- **大字标题**：标题占画面50-70%，超粗黑体，只放1-2行核心文案
- **舒适配色**：奶油色/低对比/哑光质感，避免荧光色和纯色铺满
- **杂志排版**：信息分层清晰，留白≥20%
- **人情味**：手写感/便签元素加分
- **差异化**：避免Canva模板感，每版风格应有明显区别

## 色板（6种）ID:描述
${PALETTES.map(p => `${p.id}: ${p.name} — ${p.desc}`).join(' | ')}

## 版式（6种）ID:描述
${LAYOUTS.map(l => `${l.id}: ${l.desc}`).join(' | ')}

## 避雷（绝对禁止）
❌ 荧光渐变大面积铺满 | 白色描边文字 | 3个以上emoji | 标题超12字 | 3D字效 | 不留白塞满 | 高饱和度正红/正绿铺满

## 输出（严格JSON数组，3元素，风格完全不同）
[{"title":"6-12字带emoji","subtitle":"2-8字","palette":"选一个ID","layout":"选一个ID","style_note":"15字内说明"}]

3版必须在色板、版式、风格上完全不同。第1版最主流，第2版反差，第3版大胆。`;
